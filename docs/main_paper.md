...
---
**원고 상태**: Draft v3 — GPU 실측 이전 단계, 문헌 기반 참고 데이터 사용.
**v3 변경**: 정리(Theorem)/정의(Definition) 번호 부여, 기호(notation) 표 추가, 관련 연구 확장 및 본문 인용, 위협 요소(Threats to Validity) 절, 재현성·데이터/코드 가용성 진술, 통계 부록(B) 추가. 수치는 v2와 동일(검증 도구 `experiments/verify_numbers.py` 로 60개 항목 자동 확인).
...
python analytical_proof.py               # 폐형 유도 + Jevons 기호 증명
...
---
*생성 도구: PCAG Research Pipeline. v3 마지막 갱신: 2026-09-01. (v3: 정리/정의 번호, 기호표, 관련연구+인용, 위협 요소, 재현성·가용성 진술, 통계 부록, 무결성 게이트 `verify_numbers.py`)*
